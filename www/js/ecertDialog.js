var ecertDialog = {
	...
	displayCert			: function(certifications) {
		...
		$.each(this._certItems, function(idx, certItem) {
			...
			html = '<tr tabindex="'+ idx + '">';
			html += '<td class="text-center"><i class="ico-cert"></i>' + ecertDialog._getUsetype(certItem["cert.extension.policyid"]) + '</td>';
			html += '<td>';
			html += '<div class="ellipsis">' + certItem["cert.subjectname.CN"] + '</div>';
			html += '</td>';
			html += '<td class="text-center">' + ecertDialog._convertTime(certItem["cert.validity.notAfter"])  + '</td>';
			html += '<td class="text-center">' + ecertDialog._getOrganization(certItem["cert.extension.policyid"]) + '</td>';
			html += '</tr>';
		...
		// 인증서 리스트 선택
		$('#table-certlist td').on('click keypress', function () {
			$(this).parent().addClass('selected').siblings().removeClass('selected');
		});
    },
    
	displayExtdrive 	: function(drivers) {
		this.usbItems = typeof(drivers)=="object"?drivers:[];drivers;
		
		var html = "";
		var i =0
		$.each(this.usbItems, function(idx, usbItem) {
			if(usbItem != ""){
				html += '<li tabindex="'+ idx + '">' + usbItem + "</li>";
			}
		});
    ...
    _confirmButton 		: function(e) {
		...
		//OnConfirmClick
		if (bValide) {
			var index = $("#listCert .selected").attr("tabindex");
			if (ecertDialog.externalInput) {
				var passwordobj = $("#certPassword");
			}
	...
	_createCertSelect 	: function() {
		...
		select.setAttribute("class", "cert-select");
		
		//button disk external disk
		for (var i=0; i<2; i++) {
			var imgEle = document.createElement("img");
			imgEle.setAttribute("tabindex", i);
			imgEle.setAttribute("id", (i==0?"harddisk":"usb"));
			imgEle.setAttribute("src", (i==0?"images/tab_harddisk_on.png":"images/tab_usb.png"));
			imgEle.setAttribute("alt", (i==0?"하드디스크":"이동식디스크"));
			select.appendChild(imgEle);
		}
		
		//append external dist list
		var listUSB = document.createElement("ul");
		listUSB.setAttribute("id", "listUSB");
		listUSB.setAttribute("class", "usb-list");
		listUSB.setAttribute("style", "display: none;");
		select.appendChild(listUSB);
	...
	_createCertList		: function() {
		...
		list.setAttribute("class", "cert-list");
		
		//create table
		var table = document.createElement("table");
		table.setAttribute("class", "table-bordered");
		table.setAttribute("id", 	"table-certlist");

		var header = document.createElement("thead");
		var tr = document.createElement("tr");
		
		//th create
		for (var i=0; i<this._headElement.length; i++) {
			var th = document.createElement("th");
			th.innerText = this._headElement[i];
			tr.appendChild(th);
		}
		...
		tbody.setAttribute("id", "listCert");
	...
	_createDialog		: function() {
		if (!this._certDialog)  {
			...
			//TODO IE7 화면 처리
			$(".cert-select").attr("class", "cert-select");
			$(".cert-list").attr("class", "cert-list");
			$(".password-wrapper").attr("class", "password-wrapper");
			$(".form-cert-password").attr("class", "form-cert-password");
			$(".table-bordered").attr("class", "table-bordered");
			$(".cert-list").attr("class", "cert-list");
			$(".usb-list").attr("class", "usb-list");
...
//				    ecertDialog.closeDialog();
			...
			$('.cert-select img').on('click keypress', function () {
				var isUSB = this.id === 'usb';
